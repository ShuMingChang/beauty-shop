...
const env = require(`./config/${envConfigVariable}.env`)
// const envVersion = require('./config/version.env.js')
// const env = Object.assign(envConfig, envVersion)

console.log('Start to build...')
console.log(`Information -  by config "${envConfigVariable}".`, env)

const exportConfig = merge(require(`./build/webpack.${env.BUILD_CONFIG}`), {
  /**
   * 链式操作 (高级)
   * 可参考
  ...
  chainWebpack: config => {
    ...
    config.plugin('define')
      .tap((definitions) => {
        ...
        definitions[0]['process.env'].VERSION = `"${definitions[0]['process.env'].VERSION}"`
        return definitions
      })
